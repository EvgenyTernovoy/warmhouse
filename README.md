...
1. Добавлен микросервис управления устройствами device-service, где реализован переход на новую версию api. Для тестирования:
   - запустить `./init.sh`
   - создать sensor используя postman collection
   - получить sensor по id используя postman collection "Get Sensor by ID V2"

2. Добавлен микросервис для получения телеметрии с датчиков telemetry-service.
   - запустить `./init.sh`
   - получить данные используя postman collection "Get Sensor by ID V2"